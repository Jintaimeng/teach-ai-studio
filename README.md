...
- 在项目根目录的 `mcp.json` 中配置 MCP Server，服务端会在启动时读取并传给 CodeBuddy SDK。
- 可用 Skill 会从 CodeBuddy CLI 的用户/项目配置中加载，并在对话中提示模型按用户意图自动触发对应 `/skill` 命令。
...
| `/api/mcp-status` | GET | 获取最近一次 MCP 连接状态 |
...
默认读取项目根目录的 `mcp.json`，也可以通过 `MCP_CONFIG_PATH` 指定其他路径。
...
如果出现一直“思考中”但没有输出，请先确认 `.env` 中的 `CODEBUDDY_API_KEY` 或 CLI 登录状态有效。
