...
import { Message, Model, PermissionRequest, ContentBlock, AvailableSkill, McpServerState } from '../types';
...
export function ChatMessages({ 
  ...
  const hasRuntimeMetadata = (mcpServers?.length || 0) > 0;
        ...
        >
          {mcpServers && mcpServers.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <span style={{ color: 'var(--td-text-color-primary)' }}>MCP:</span>
              {mcpServers.map(server => (
                <span key={server.name} className="rounded-full px-2 py-0.5" style={{ backgroundColor: 'var(--td-bg-color-component)' }}>
