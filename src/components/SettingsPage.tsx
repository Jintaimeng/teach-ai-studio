...
import { CustomAgent, PermissionMode, McpServerInfo } from '../types';
...
export function SettingsPage({ 
  ...
  const [switchingMcpServers, setSwitchingMcpServers] = useState<Set<string>>(new Set());
  ...
  const fetchMcpServers = useCallback(async (force = false) => {
    ...
    try {
      const response = await fetch(`/api/mcp/servers${force ? '?force=1' : ''}`);
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || '获取 MCP 列表失败');
      }

      setMcpServers(data.servers || []);
      setMcpConfigPath(data.configPath || '');
    } catch (error: any) {
      MessagePlugin.error(error?.message || '获取 MCP 列表失败');
    } finally {
  ...
  const openMcpEditor = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || '读取 MCP 配置失败');
      }

      setMcpConfigText(data.content || '{\n  "mcpServers": {}\n}');
      setMcpConfigPath(data.path || '');
    } catch (error: any) {
      MessagePlugin.error(error?.message || '读取 MCP 配置失败');
    } finally {
  ...
  const saveMcpConfig = async () => {
    ...
    try {
      const response = await fetch('/api/mcp/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content: mcpConfigText }),
      });
      const data = await response.json();

      if (!response.ok || !data.success) {
        throw new Error(data.error || '保存 MCP 配置失败');
      }
      ...
      MessagePlugin.error(error?.message || '保存 MCP 配置失败');
    } finally {
      setSavingMcpConfig(false);
    }
  };

  const toggleMcpServer = async (server: McpServerInfo, enabled: boolean) => {
    setSwitchingMcpServers(prev => new Set(prev).add(server.name));
    try {
      const response = await fetch(`/api/mcp/servers/${encodeURIComponent(server.name)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || '切换 MCP 服务失败');
      }

      MessagePlugin.success(`${server.name} 已${enabled ? '开启' : '关闭'}`);
      fetchMcpServers(true);
    } catch (error: any) {
      MessagePlugin.error(error?.message || '切换 MCP 服务失败');
    } finally {
      setSwitchingMcpServers(prev => {
  ...
  const getMcpStatusColor = (server: McpServerInfo) => {
  ...
  const formatMcpStatus = (server: McpServerInfo) => {
    if (!server.enabled || server.status === 'disabled') return '关闭';
    if (server.status === 'connected') return '连接成功';
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div>
          ...
          {loadingMcpServers ? (
            ...
            >
              当前没有 MCP 配置。点击“编辑 mcp.json”添加新的 MCP Server。
            </div>
          ) : (
            <div className="space-y-2">
              {mcpServers.map(server => {
                const expanded = expandedMcpServer === server.name;
                return (
                  <div
                    key={server.name}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 p-3">
                      <button
                        ...
                        onClick={() => setExpandedMcpServer(expanded ? null : server.name)}
                      >
                        <span
                          className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                          style={{ backgroundColor: getMcpStatusColor(server) }}
                        />
                        <span
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <Tooltip content={server.enabled ? '关闭 MCP' : '开启 MCP'}>
                          <Switch
                            size="small"
                            value={server.enabled}
                            label={['开', '关']}
                            loading={switchingMcpServers.has(server.name)}
                            onChange={(value) => toggleMcpServer(server, Boolean(value))}
                          />
                        </Tooltip>
                        <Tag size="small" variant="outline">
      ...
      <Dialog
        ...
        confirmBtn="保存"
        cancelBtn="取消"
        onClose={() => setShowMcpEditor(false)}
        onCancel={() => setShowMcpEditor(false)}
        onConfirm={saveMcpConfig}
        confirmLoading={savingMcpConfig}
      >
        <div className="space-y-3">
          {mcpConfigPath && (
            <div 
          ...
          {loadingMcpConfig ? (
            <div className="flex items-center gap-2 py-6">
              <Loading size="small" />
              <span style={{ color: 'var(--td-text-color-secondary)' }}>正在读取配置...</span>
            </div>
          ) : (
            <Textarea
              value={mcpConfigText}
              onChange={(value) => setMcpConfigText(value as string)}
          ...
          <div 
            ...
            style={{ color: 'var(--td-text-color-placeholder)' }}
          >
            可在单个 MCP Server 配置中添加 "disabled": true 关闭该服务。
          </div>
        </div>
      </Dialog>
    </div>
